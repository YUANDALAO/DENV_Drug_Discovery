cat > complete_ultimate_analysis.py << 'EOF'
#!/usr/bin/env python3
"""
REINVENT4 完整终极分析工具
- 正确的金标准筛选（使用raw值）
...
<head>
    ...
    <title>REINVENT4 Complete Analysis</title>
    <style>
...
<body>
<div class="container">
    <h1>🧬 REINVENT4 Complete Multi-Run Analysis</h1>
    <p style="text-align: center; color: #7f8c8d; margin-bottom: 20px;">
        Generated: {self.timestamp} | Mobile & Desktop Optimized
    </p>
"""
    ...
    <div class="gold-box">
        <h3>⭐ 金标准分子定义</h3>
        <p style="margin-bottom: 15px;">必须<strong>同时满足</strong>以下所有条件：</p>
        <div class="criterion">✓ pIC50 ≥ 8.0 (IC50 ≤ 10 nM)</div>
        <div class="criterion">✓ QED ≥ 0.7 (类药性)</div>
        <div class="criterion">✓ SA Score ≤ 4.0 (合成可及性)</div>
        <div class="criterion">✓ MW: 300-500 Da (分子量)</div>
        <div class="criterion">✓ LogP: 1-4 (亲脂性)</div>
    </div>
"""
    ...
    <div class="metric-grid">
        ...
        <div class="metric-card">
            ...
            <div class="metric-value">{(total_gold/total_orig*100) if total_orig > 0 else 0:.1f}%</div>
        </div>
    </div>
"""
        return html
...
"""
        ...
        for a in self.analyzers:
            ...
            if a.config.get('has_toxicity'):
                tox_comps = ', '.join(a.config.get('toxicity_components', []))[:20]
            ...
            <tr class="{row_class}">
                ...
                <td>{avg_pic50:.2f if gold_mols else '—'}</td>
                <td>{avg_qed:.2f if gold_mols else '—'}</td>
                <td>{avg_sa:.2f if gold_mols else '—'}</td>
...
"""
    ...
    <p style="background: #e3f2fd; padding: 15px; border-radius: 8px;">
        <strong>💡 结论:</strong> 毒性组件使金标准数量变化 <strong>{(avg_with-avg_without)/avg_without*100 if avg_without > 0 else 0:+.1f}%</strong>
    </p>
"""
    ...
    <p style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
        <strong>总计:</strong> 所有runs共产生 <strong>{len(all_scaffolds)}</strong> 个不同的Murcko骨架
    </p>
    
    <table>
        <thead>
            <tr><th>Run</th><th>独特骨架数</th><th>金标准数</th><th>多样性比</th></tr>
        </thead>
        <tbody>
"""
...
"""
    ...
    <div style="margin-top: 60px; padding-top: 30px; border-top: 3px solid #3498db; text-align: center; color: #7f8c8d;">
        ...
        <p style="font-size: 0.9em; margin-top: 10px;">
            With Gold Standard Filtering, Toxicity Analysis & Scaffold Diversity
        </p>
        <p style="font-size: 0.85em; margin-top: 15px;">
            📱 Mobile & Desktop Optimized | 🔬 Research Grade
        </p>
    </div>
</div>
</body>
...
chmod +x complete_ultimate_analysis.py
python complete_ultimate_analysis.py