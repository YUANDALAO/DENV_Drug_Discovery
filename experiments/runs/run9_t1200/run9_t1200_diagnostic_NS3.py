...
def load_training_data():
    ...
    print(f"读取文件: {BACKUP_TRAINING_FILE}")
    try:
        # 指定encoding处理BOM
        ...
        if 'Smiles' not in df.columns:
            print(f"✗ 错误: 找不到Smiles列")
            print(f"  可用列: {df.columns.tolist()}")
            return None
        
        smiles_list = df['Smiles'].dropna().tolist()
        print(f"✓ 读取了 {len(smiles_list)} 个SMILES")
        print(f"  示例: {smiles_list[0][:60]}...")
        
        # 🔴 关键：只做基本清洗，不要超严格筛选
        clean_smiles = []
        
        for smi in tqdm(smiles_list, desc="清洗训练集"):
            if pd.isna(smi):
                continue
        ...
        print(f"✓ 清洗后: {len(unique_smiles)} 个有效SMILES")
        ...
        # 显示第一个分子确认
        print(f"  第1个: {unique_smiles[0][:60]}...")
